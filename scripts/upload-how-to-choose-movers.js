...
function he(str) {
  return str.split('').map(c => {
    ...
    if (code > 127) return `\\u{${code.toString(16).toUpperCase()}}`;
    return c;
  }).join('');
}
...
<div class="qbox">
מוביל טוב מוביל דירה בלי לשבור ריהוט, בלי הפתעות במחיר, ועם ביטוח על המטען. כדי לבחור נכון — <strong>בדקו רישיון, ביטוח, חוות דעת, ופירוט מחיר בכתב</strong> לפני שמשלמים שקל.
</div>
...
<div class="checklist">
<div class="chk"><div class="chk-num">1</div><div class="chk-body"><h3>רישיון ותעודת מוביל</h3><p>בישראל מוביל חייב ברישיון מטעם משרד התחבורה. בקשו לראות תעודה. מוביל ללא רישיון = סיכון.</p></div></div>
<div class="chk"><div class="chk-num">2</div><div class="chk-body"><h3>ביטוח מטען</h3><p>שאלו ישירות: "יש לכם ביטוח על המטען?". מוביל רציני ישיב בחיוב. שבירת ריהוט יקר קורה — הביטוח חוסך עוגמת נפש.</p></div></div>
<div class="chk"><div class="chk-num">3</div><div class="chk-body"><h3>חוות דעת אמיתיות</h3><p>חפשו שם המוביל ב-Google Maps וב-LiftyGo. ביקורות עם פרטים ספציפיים (גודל דירה, עיר, בעיה שנפתרה) = אמינות.</p></div></div>
<div class="chk"><div class="chk-num">4</div><div class="chk-body"><h3>מחיר מפורט בכתב</h3><p>ודאו שהצעת המחיר כוללת: עלות בסיס, דלק, מספר פועלים, פירוק/הרכבה, קומות. מוביל שמסרב לכתוב — עברו הלאה.</p></div></div>
<div class="chk"><div class="chk-num">5</div><div class="chk-body"><h3>ניסיון עם הפריטים שלכם</h3><p>יש פסנתר? כספת? ארונות מורכבים? שאלו מראש. לא כל מוביל מתאים לכל עבודה.</p></div></div>
</div>
...
<li>לא מוכן לתת הצעת מחיר בכתב</li>
<li>אין מספר טלפון קבוע — רק וואטסאפ</li>
<li>אין חוות דעת ברשת מהשנה האחרונה</li>
...
<tbody>
<tr><td>מובילים מאומתים</td><td>✅ כולם עברו בדיקה</td><td>❌ אחריות עליכם</td></tr>
<tr><td>השוואת מחירים</td><td>✅ 3 הצעות בקליק</td><td>❌ שיחות טלפון רבות</td></tr>
<tr><td>חוות דעת</td><td>✅ מובנות בפלטפורמה</td><td>❌ פזורות ברשת</td></tr>
<tr><td>ביטוח</td><td>✅ דרישת חובה</td><td>❌ תלוי במוביל</td></tr>
<tr><td>זמן חיפוש</td><td>✅ 3 דקות</td><td>❌ שעות</td></tr>
</tbody>
</table></div>

<div class="cta-box">
<h3>קבלו 3 הצעות מחיר ממובילים מאומתים</h3>
<p>שאלון קצר → מובילים מגיעים אליכם → בוחרים ונעים. ללא עמלה, ללא התחייבות.</p>
...
async function run() {
  ...
  const newPost = await apiReq('POST', '/wp-json/wp/v2/posts', {
    title: 'איך לבחור מוביל דירה — המדריך המלא 2026',
    slug: 'how-to-choose-movers',
    status: 'publish',
    categories: [5, 7],
  ...
  if (!newPost.id) {
    console.error('Failed to create post:', JSON.stringify(newPost).substring(0, 400));
    return;
  }
  const postId = newPost.id;
  console.log('✓ Post created — ID:', postId, '| URL:', newPost.link);

  // ── 2. Apply Elementor template ──────────────────────────────────────
  console.log('Applying Elementor template...');
  ...
  const heroWidget = {
    ...
    settings: {
      html: makeHeroHTML(
        title, 'מדריך לקוח', 'moving-guides', 'מאי 2026', 6,
  ...
  const upd = await apiReq('PATCH', '/wp-json/wp/v2/posts/' + postId, {
    meta: {
  ...
  if (upd.id) console.log('✓ Elementor applied | link:', upd.link);
  else { console.error('✗ Template failed:', JSON.stringify(upd).substring(0,300)); return; }

  // ── 3. Update noindex.php local + push snippet 5 ─────────────────────
  console.log('Updating snippet 5 (noindex)...');
  ...
  noindexCode = noindexCode.replace(
    ...
    `        2894, // כמה עולה הובלת דירה — ממתין לאישור אינדוקס\n        ${postId}, // איך לבחור מוביל דירה — ממתין לאישור אינדוקס`
  );
  ...
  console.log('✓ Snippet 5 pushed — active:', snap5.active);

  // ── 4. Update meta-tags.php local + push snippet 8 ───────────────────
  console.log('Updating snippet 8 (meta-tags)...');
  const metaPath = 'C:/Users/itay/liftygo-site/seo/snippets/meta-tags.php';
  let metaCode = fs.readFileSync(metaPath, 'utf8');

  const titleHe = he('איך לבחור מוביל דירה — המדריך המלא 2026 | LiftyGo');
  const descHe  = he('איך לבחור מוביל דירה? 5 דברים חייבים לבדוק: רישיון, ביטוח, חוות דעת ומחיר בכתב. קבלו 3 הצעות ממובילים מאומתים.');
  const ogTitleHe = he('איך לבחור מוביל דירה — המדריך המלא | LiftyGo');

  // Insert into $custom_titles before 2894 entry
  metaCode = metaCode.replace(
    '        2894 => "\\u{05DB}',
    `        ${postId} => "${titleHe}",\n        2894 => "\\u{05DB}`
  );

  // Insert into $pages[] before 2894 entry
  const pagesEntry = `        ${postId} => [\n            "desc"     => "${descHe}",\n            "og_title" => "${ogTitleHe}",\n            "og_url"   => "https://liftygo.co.il/how-to-choose-movers/",\n        ],\n`;
  metaCode = metaCode.replace(
    '        2894 => [\n',
    pagesEntry + '        2894 => [\n'
  );
  ...
  console.log('✓ Snippet 8 pushed — active:', snap8.active);

  // ── 5. Send WhatsApp ─────────────────────────────────────────────────
  console.log('Sending WhatsApp...');
  const waMsg = JSON.stringify({
    chatId: '972526660006@c.us',
    message: `מאמר חדש עלה לאוויר! ✍️\n\nכותרת: איך לבחור מוביל דירה — המדריך המלא 2026\nלינק: https://liftygo.co.il/how-to-choose-movers/\nמילת מפתח: איך לבחור מוביל דירה\n\nהמאמר כרגע noindex — בדוק ואשר. השב "תעלה" לאינדקס.`
  });
  ...
  await new Promise(r => {
    ...
    }, res => { let d=''; res.on('data',c=>d+=c); res.on('end',()=>{ console.log('✓ WhatsApp:', d.substring(0,80)); r(); }); });
    req.on('error', e => { console.error('WA error:', e.message); r(); });
    req.write(waMsg);
    req.end();
  });
  ...
  console.log('URL: https://liftygo.co.il/how-to-choose-movers/');
  console.log('\nNext: git add + commit + push, then update blog page');
}
